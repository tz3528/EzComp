...
private:
	...
	/// 生成comp.solve操作，包含init/boundary/step三个region
	...
	// points/delta 更适合作为表达式层 helper（用到前置生成）
	...
	llvm::StringMap<mlir::Value> eqValue;           // 表达式文本 -> 计算结果

	/// 时间循环上下文
	struct TimeLoopCtx {
		comp::ForTimeOp loop;
		mlir::Value atTime, writeTime;
